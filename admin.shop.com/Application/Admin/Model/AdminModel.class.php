...
class AdminModel extends \Think\Model {
    ...
    public function getPageResult(array $cond = array()) {
        ...
        return compact('rows', 'page_html');//创建包含变量名和它们的值的数组
    }
    ...
    public function addAdmin() {
        $this->startTrans();//涉及多张表,开启事物,减少脏数据
        $this->data['token'] = \Org\Util\String::randString(32);//获得32位字符串
        $this->data['token_create_time']=NOW_TIME;//获取当前时间戳
        $this->data['password'] = salt_mcrypt($this->data['password'], $this->data['salt']);//自动完成时salt已经生成
        $admin_id=$this->add();
        if($admin_id==false){
            $this->rollback();//回滚
            return false;
    ...
    public function getAdminInfo($id) {
        ...
        $row = $this->find($id);
        $row['role_id']=json_encode($role_ids);
        return $row;
    }
